...
do
  ...
  local function is_real_file_buffer(buf)
    ...
    if vim.bo[buf].buftype ~= '' then return false end -- excluye nofile, terminal, etc.
  ...
  vim.keymap.set('n', '<leader>e', function()
    if vim.bo.filetype == 'netrw' then
      if netrw_back and is_real_file_buffer(netrw_back) then
        vim.api.nvim_set_current_buf(netrw_back)
      else
        vim.cmd 'ene' -- recién abierto: no había archivo previo real
      end
      return
    end

    local cur = vim.api.nvim_get_current_buf()
    if is_real_file_buffer(cur) then
      netrw_back = cur
    else
      netrw_back = nil
    end
    ...
    vim.cmd 'e .'
  end, { desc = 'Open netrw; return to previous file (or new buffer)' }) -- Clear highlights on search when pressing <Esc> in normal mode
  --  See `:help hlsearch`
